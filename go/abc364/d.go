...
func solve() {
	n, q := iost.GetNextInt(), iost.GetNextInt()
	aa := make([]int, n)
	for i := 0; i < n; i++ {
	...
	bk := make([][2]int, q)
	for i := 0; i < q; i++ {
		bk[i][0], bk[i][1] = iost.GetNextInt(), iost.GetNextInt()
	}
	sort.Ints(aa)
	for i := 0; i < q; i++ {
		b, k := bk[i][0], bk[i][1]
		iost.Println(findKthClosest(aa, b, k))
	}
}

func findKthClosest(nums []int, x, k int) int {
